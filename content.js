var lineup_info = [["Draft_ID", 'Pick_Position', 'Build', 'Name', 'Position', 'Team', 'ADP' ,'Pick']]

var tourneyName = document.querySelector(".styles__tournamentTitle___PrZU").textContent.trim();
...
async function get_exposures() {
    ...
    for (var i = 0; i < entries.length; i++) {
        ...
        let wr = build[2].textContent.trim();
        let te = build[3].textContent.trim();
        let build_list = `${qb}-${rb}-${wr}-${te}`;
        ...
        for (position = 0; position < positions.length; position++) {
            ...
            for (var player = 0; player < players.length; player++) {
                ...
                var adp = add_info[0].textContent.trim();
                var pick = add_info[1].textContent.trim();
